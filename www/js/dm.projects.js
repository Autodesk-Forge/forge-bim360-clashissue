...
class DMProjects { 
  ...
  async refreshBIMHubs() { 
    ...
    $('#projectview').html(
      ...
      +'<div class="fa-middle-modelset"><i id="msSpinner" class="fa fa-spinner fa-spin fa-5x" style="display:none;margin: auto"></i></div>'
      +'</div>'
    );  
    var _this = this
    hubs.forEach( (ele)=> {  
    ...
    projects.forEach( (ele) =>{
          if (ele.type === 'bim360projects')  
              ...
              $('#'+validHtmlHubId).append(
                 ...
                 + 'id="'+ validhtmlid  + '"' + ' data="' +validHtmlHubId +'"'
  ...
  async refreshProjectInfo(hubId,projectId){
    return new Promise(( resolve, reject ) => { 
      $.ajax({
        url: '/dm/refreshProjectInfo/'+hubId+'/'+projectId,
        type: 'GET' ,
        success: function (res) {  
            resolve(res) 
