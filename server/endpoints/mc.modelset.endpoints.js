...
router.get('/mc/modelset/getModelSets/:mc_container_id', async (req, res, next) => {
  ...
  try {
      ...
      let msArray = await getMSVersionCommonInfo(input,mssRaw.modelSets)   
...
async function getMSVersionCommonInfo(input,modelSets){

  var msArray = []   
  await Promise.all(modelSets.map(async (each_ms) => {
        input.ms_id = each_ms.modelSetId
        let r = await mcMSServices.getModelSet(input)
        if(!r.isDisabled){
          msArray.push({ms_id:each_ms.modelSetId,
                        ms_name:each_ms.name,
