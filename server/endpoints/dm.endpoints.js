...
router.get('/dm/refreshProjectInfo/:hubId/:projectId', async (req, res, next) => {
  try{
    const userSession = new UserSession(req.session);
    if (!userSession.isAuthorized()) {
      res.status(401).end('Please login first');
    ...
    //for refresh the basic data of a project. nothing is required to send to client at this moment
    res.end()
  }
  catch(ex){
    console.log('Get BIM projects failed! ' + ex.toString())
